...
def Status(shit):
    print(Fore.YELLOW +"[ Dark4Hackers]" + Style.RESET_ALL
    + " " + shit)
...
def DoitAnakin(reportedGuy,reportedGuyID,username,password,proxy):
    try:
        ...
        if insta.loggedIn == True and result == True:
            ...
            Fail(username,"Giriş yapıldı ancak spam atılmadı (hesap patladı)")
        elif insta.loggedIn == False:
            Fail(username,"Yanlış kullanıcı adı veya şifre")
    except:
...
if __name__ == "__main__":
    ...
    reportedGuy = input(Fore.GREEN + "kullanıcı adı: " + Style.RESET_ALL)
    reportedGuyID = input(Fore.GREEN + "User Id: " + Style.RESET_ALL)
    print("")
    Status("Spamlama Başladı\n")
